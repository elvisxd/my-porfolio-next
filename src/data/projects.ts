...
const projectsData: TranslatableProject[] = [
  ...
  },
  {
    ...
    title: {
      ...
      es: "Trading Spot — Avisos de Zonas de Acumulación",
    },
    description: {
      en: "A research and execution system I build and operate on my own. It started as a 283k-line futures engine with 34 strategies, a multi-factor signal core and ~136 backtest scripts. After eleven rounds of measurement — permutation tests against reshuffled whole periods, thirds validation, frozen out-of-sample splits — I concluded that none of the modules beat their benchmark under real execution, and I retired the entire system rather than keep trading it. What survives is what passed that same bar: a multi-year range map that alerts over Telegram when an asset moves between zones of its own range, plus a market panel to inspect it. The alerts state explicitly that they inform rather than recommend, because the buy-cheap and scaled-sell criteria were measured too and did not beat their benchmarks either — that finding is kept in the code so nobody retries it thinking it is new. A Gemini integration summarizes filings through a REST client with timeout, temperature and token budgeting, degrading gracefully: if the model fails, the product keeps running without it. The alerting logic is a single shared function, so what gets alerted and what gets drawn can never drift apart.",
      es: "Sistema propio de investigación y ejecución que construyo y opero solo. Empezó como un motor de futuros de 283k líneas con 34 estrategias, un núcleo de señales multifactor y ~136 scripts de backtest. Tras once rondas de medición —test de permutación barajando periodos enteros, validación por tercios y out-of-sample congelado— concluí que ninguno de los módulos batía a su benchmark bajo ejecución real, y retiré el sistema entero en vez de seguir operándolo. Lo que sobrevive es lo que pasó esa misma vara: un mapa de rango plurianual que avisa por Telegram cuando un activo cambia de zona dentro de su propio recorrido, más un panel de mercado para mirarlo. Los avisos dicen explícitamente que informan y no recomiendan, porque los criterios de compra barata y venta escalonada también se midieron y tampoco batían a sus benchmarks: ese hallazgo se conserva en el código para que nadie lo reintente creyendo que es nuevo. Una integración con Gemini resume documentos mediante un cliente REST propio con control de timeout, temperatura y presupuesto de tokens, con degradación elegante: si el modelo falla, el producto sigue funcionando sin él. La lógica de avisos es una única función compartida, así que lo que se avisa y lo que se pinta no pueden separarse.",
    },
    shortDescription: {
